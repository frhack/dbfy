...
fn detect_jsonl(path: &Path, sample: usize) -> Result<ParserBody> {
    ...
    for line in reader.lines() {
        ...
        let value: Value = serde_json::from_str(&line)
            .with_context(|| format!("invalid JSON on line {}", count))?;
...
fn render_yaml(path: &Path, opts: &DetectOpts, body: ParserBody) -> String {
    ...
    out.push_str(
        "# Generated by `dbfy detect`. Tweak names, types, and indexed_columns as needed.\n",
    );
...
mod tests {
    ...
    fn jsonl_inference_promotes_mixed_numerics_to_float() {
        ...
        assert!(
            body.parser_yaml
                .contains("name: x, path: \"$.x\", type: float64"),
            "yaml was: {}",
            body.parser_yaml
        );
    ...
    fn csv_inference_uses_header_and_picks_int_for_pure_int_column() {
        ...
        assert!(
            body.parser_yaml
                .contains("name: id, source: { name: id }, type: int64")
        );
        assert!(
            body.parser_yaml
                .contains("name: score, source: { name: score }, type: float64")
        );
        assert!(
            body.parser_yaml
                .contains("name: name, source: { name: name }, type: string")
        );
    }
